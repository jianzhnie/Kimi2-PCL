...
def load_config_module():
    ...
    spec_config = importlib.util.spec_from_file_location(
        "models.configuration_deepseek", models_dir / "configuration_deepseek.py"
    )
...
def compute_weight_shape(name: str, config: DeepseekV3Config) -> Tuple[List[int], str]:
    """
    ...
    if name.startswith("model.layers."):
        ...
        if is_moe_layer:
            ...
            if "mlp.experts." in name and ".gate_proj.weight" in name:
                return [config.moe_intermediate_size, config.hidden_size], dtype
            
            if "mlp.experts." in name and ".up_proj.weight" in name:
                return [config.moe_intermediate_size, config.hidden_size], dtype
            
            if "mlp.experts." in name and ".down_proj.weight" in name:
                return [config.hidden_size, config.moe_intermediate_size], dtype
...
def generate_weight_map(config: DeepseekV3Config) -> Dict:
    ...
    # Embedding weights
    add_weight("model.embed_tokens.weight", [config.vocab_size, config.hidden_size])
    ...
    for layer_idx in range(config.num_hidden_layers):
        ...
        add_weight(f"{prefix}.post_attention_layernorm.weight", [config.hidden_size])
        ...
        if is_moe_layer:
            # Gate weights
            add_weight(f"{prefix}.mlp.gate.weight", [config.n_routed_experts, config.hidden_size])
            
            if getattr(config, 'moe_router_enable_expert_bias', False):
                add_weight(f"{prefix}.mlp.gate.bias", [config.n_routed_experts])
            
            if config.topk_method == 'noaux_tc':
                add_weight(f"{prefix}.mlp.gate.e_score_correction_bias", [config.n_routed_experts])
            
            # Expert weights
            for expert_idx in range(config.n_routed_experts):
                expert_prefix = f"{prefix}.mlp.experts.{expert_idx}"
                add_weight(f"{expert_prefix}.gate_proj.weight", [config.moe_intermediate_size, config.hidden_size])
                add_weight(f"{expert_prefix}.up_proj.weight", [config.moe_intermediate_size, config.hidden_size])
                add_weight(f"{expert_prefix}.down_proj.weight", [config.hidden_size, config.moe_intermediate_size])
            ...
            if config.n_shared_experts is not None and config.n_shared_experts > 0:
                ...
                add_weight(f"{shared_prefix}.gate_proj.weight", [shared_intermediate, config.hidden_size])
                add_weight(f"{shared_prefix}.up_proj.weight", [shared_intermediate, config.hidden_size])
                add_weight(f"{shared_prefix}.down_proj.weight", [config.hidden_size, shared_intermediate])
        else:
            # Dense MLP
            add_weight(f"{prefix}.mlp.gate_proj.weight", [config.intermediate_size, config.hidden_size])
            add_weight(f"{prefix}.mlp.up_proj.weight", [config.intermediate_size, config.hidden_size])
            add_weight(f"{prefix}.mlp.down_proj.weight", [config.hidden_size, config.intermediate_size])
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract weight information from DeepseekV3 model definition"
    )
    parser.add_argument(
        "--output",
        "-o",
