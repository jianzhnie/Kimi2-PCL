...
def create_dense_mlp_weights(
    ...
    return {
        f"{prefix}.gate_proj.weight": torch.randn(intermediate_size,
                                                  hidden_size),
        f"{prefix}.up_proj.weight": torch.randn(intermediate_size,
                                                hidden_size),
        f"{prefix}.down_proj.weight": torch.randn(hidden_size,
                                                  intermediate_size),
    }
...
def save_hf_model(
    ...
    # Create index
    weight_map = {
        k: 'model-00001-of-00001.safetensors'
...
def create_moe_hf_model(model_dir: str) -> None:
    ...
    for layer_idx in range(first_k_dense_replace, 4):
        ...
        weights.update(create_attention_weights(layer_idx, dims))
        weights.update(
            create_moe_weights(layer_idx, hidden_size, num_experts,
                               moe_ffn_hidden_size))
...
def run_convert_command(
        cmd: list,
        timeout: int = 300) -> tuple[bool, subprocess.CompletedProcess]:
