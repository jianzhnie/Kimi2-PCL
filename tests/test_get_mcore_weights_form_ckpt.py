#!/usr/bin/env python3
"""
测试 get_mcore_weights_form_ckpt.py 的功能
"""
...
from utils.get_mcore_weights_form_ckpt import (
...
class TestGetMCoreWeightsFormCkpt(unittest.TestCase):
    """测试 get_mcore_weights_form_ckpt.py 的功能"""
    ...
    def test_get_tensor_size(self):
        ...
        size = _get_tensor_size((10, 20), 'float16')
        self.assertEqual(size, 10 * 20 * 2)
        
        # float32: 4 bytes per element
        size = _get_tensor_size((10, 20), 'float32')
        self.assertEqual(size, 10 * 20 * 4)
        
        # bfloat16: 2 bytes per element
        size = _get_tensor_size((10, 20), 'bfloat16')
        self.assertEqual(size, 10 * 20 * 2)
    ...
    def test_mcore_name_format(self):
        ...
        self.assertIn('module.decoder.layers.0.input_layernorm.weight', megatron_params)
        self.assertIn('module.decoder.layers.0.self_attention.linear_qkv.weight', megatron_params)
    ...
    def test_tp_weight_merging(self):
        ...
        embed_shape = megatron_params['module.embedding.word_embeddings.weight']['shape']
        self.assertEqual(embed_shape[0], 100, 
                        f"Embedding 第0维应该合并为 100，实际是 {embed_shape[0]}")
        
        # 验证 output_layer 的形状已合并
        output_shape = megatron_params['module.output_layer.weight']['shape']
        self.assertEqual(output_shape[0], 100,
                        f"Output layer 第0维应该合并为 100，实际是 {output_shape[0]}")

    def test_layer_index_conversion(self):
        """测试 layer index 转换（PP 场景）"""
        # 创建 PP=2, 共 4 层的 checkpoint
        self.create_mock_checkpoint(tp_size=1, pp_size=2, ep_size=1, num_layers=4)
        ...
        layer_ids = set()
        for name in megatron_params.keys():
            match = __import__('re').match(r'module\.decoder\.layers\.(\d+)\.', name)
            if match:
                layer_ids.add(int(match.group(1)))
        
        self.assertEqual(sorted(layer_ids), [0, 1, 2, 3],
                        f"应该提取所有 4 层的权重，实际是: {sorted(layer_ids)}")
    ...
    def test_fused_weights_not_expanded(self):
        ...
        # 验证 QKV 是融合权重，没有被展开
        self.assertIn('module.decoder.layers.0.self_attention.linear_qkv.weight', megatron_params)
        # 不应该有单独的 q_proj, k_proj, v_proj
        self.assertNotIn('module.decoder.layers.0.self_attn.q_proj.weight', megatron_params)
        
        # 验证 MLP 是融合权重，没有被展开
        self.assertIn('module.decoder.layers.0.mlp.linear_fc1.weight', megatron_params)
        # 不应该有单独的 gate_proj, up_proj
        self.assertNotIn('module.decoder.layers.0.mlp.gate_proj.weight', megatron_params)
    ...
    def test_vpp_layer_mapping(self):
        ...
        # 创建 8 层的 VPP checkpoint，PP=2，VPP=2
