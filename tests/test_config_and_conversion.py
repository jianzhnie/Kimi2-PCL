...
def test_mcore2hf_set_layer_attn_shapes():
    ...
    for tp in range(conv.tp_size):
        models[(tp, 0)] = {
            "decoder.layers.0.self_attention.linear_qkv.weight": torch.zeros(rows, hidden),
            "decoder.layers.0.self_attention.linear_proj.weight": torch.zeros(hidden, 16),
        }
...
def test_hf2mcore_set_layer_attn_shapes():
    ...
    conv._set_layer_attn(hf_layer=0, local_layer_idx=0, weights=weights, mg_model=mg_model)
    ...
    proj_0 = mg_model[0][0]["decoder.layers.0.self_attention.linear_proj.weight"]
    proj_1 = mg_model[0][1]["decoder.layers.0.self_attention.linear_proj.weight"]
