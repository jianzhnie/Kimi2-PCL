...
#### 3.1.3  架构 对比 DeepSeek V3 / Qwen3

##### 1. 注意力机制退化：从 MLA 倒退至极端 GQA

- **缺陷描述**：DeepSeek V3 的核心创新是 **MLA (Multi-head Latent Attention)**，通过低秩压缩 KV 缓存实现极高效率。PCL-Model-1T 弃用了 MLA，转而采用 **32:1 的极端 GQA (Grouped Query Attention)** 比例（64 Query : 2 KV heads）。
- **技术对比**：
  | 维度            | DeepSeek V3     | **PCL-Model-1T**             | Qwen3            |
  | :-------------- | :-------------- | :--------------------------- | :--------------- |
  | **注意力方案**  | MLA (低秩压缩)  | **极端 GQA (32:1)**          | GQA (8:1 ~ 16:1) |
  | **KV 压缩质量** | 无损/极低损压缩 | **暴力砍头 (Heads Pruning)** | 适度压缩         |
  | **推理开销**    | 极低            | **极低 (但以精度为代价)**    | 中等             |
- **潜在风险**：32:1 的压缩比会导致严重的注意力信息丢失，削弱模型在长文本理解、代码逻辑推理等对细粒度注意力敏感任务上的表现。

##### 2. 深度-宽度比极度失调 (Depth-Width Imbalance)

- **缺陷描述**：模型仅 32 层，但隐藏维度高达 7168。在 LLM 设计中，这种“浅而宽”的结构通常被认为不如“深而窄”的结构有效。
- **参数规模对比**：
  | 模型             | 层数   | 隐藏维度 | 参数量级      | 架构特征     |
  | :--------------- | :----- | :------- | :------------ | :----------- |
  | DeepSeek V3      | 61     | 7168     | 671B          | 均衡型       |
  | Qwen3-235B       | 94     | 8192     | 235B          | 深层型       |
  | **PCL-Model-1T** | **32** | **7168** | **~1T (MoE)** | **极端浅层** |
- **负面影响**：
  - **表征能力受限**：层数决定了抽象层次，32 层难以支撑 1T 参数量级应有的深层逻辑推理能力。
  - **学习效率低**：训练时梯度传播路径过短，模型可能倾向于学习浅层特征而非深层语义。

##### 3. MoE 激活专家过于稀疏 (Over-Sparse Routing)

- **缺陷描述**：在 128 个专家中仅激活 2 个 (`num_experts_per_tok=2`)，激活率仅为 **1.56%**，远低于行业主流水平。
- **激活率对比**：
  | 模型             | 总专家数 | Top-K 激活 | 激活率    |
  | :--------------- | :------- | :--------- | :-------- |
  | DeepSeek V3      | 256      | 8          | 3.13%     |
  | Qwen3 MoE        | 128      | 8          | 6.25%     |
  | **PCL-Model-1T** | **128**  | **2**      | **1.56%** |
- **负面影响**：单 Token 接触的知识面过窄，限制了专家的协同效应和跨领域泛化能力，容易导致训练时的负载不均和专家坍缩。
