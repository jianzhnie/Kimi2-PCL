...
launch_on_node() {
    ...
    local array_decl
    array_decl=$(build_vllm_args_declare "${is_headless}" "${node_rank}" "${dp_start_rank}" "${dp_size_local}" "${master_addr}" "${nnodes}" "${vllm_port}")
...
deploy_standard() {
    ...
    if [[ ${CARDS_PER_INSTANCE} -gt ${NPUS_PER_NODE} ]]; then
        # ----------------------------------------------------------------------
        # 场景 A: 单个模型实例跨多个节点 (需要 --nnodes / --node-rank / --master-addr)
        # 参考: https://docs.vllm.ai/en/stable/serving/parallelism_scaling/#running-vllm-with-multiprocessing
        # ----------------------------------------------------------------------
        log_info "Mode: multi-node per instance (${NODES_PER_INSTANCE} nodes per instance)"
        ...
        for ((dp_idx = 0; dp_idx < DP_SIZE; dp_idx++)); do
            ...
            log_info "Deploying DP instance ${dp_idx}/${DP_SIZE} on nodes ${instance_start_node}..$((instance_start_node + NODES_PER_INSTANCE - 1)), master=${instance_master_node} (${instance_master_ip})"
            ...
            for ((offset = 0; offset < NODES_PER_INSTANCE; offset++)); do
                ...
                # 只有全局 Node0 启动 API server；其余所有节点（包括其他实例的 master）均 headless
                if [[ ${node_idx} -gt 0 ]]; then
                    is_headless="true"
                fi

                launch_on_node "${node}" "${local_ip}" "${is_headless}" "${offset}" "${dp_idx}" "1" "${instance_master_ip}" "${NODES_PER_INSTANCE}" "${VLLM_PORT}"
            done
        done
    else
        # ----------------------------------------------------------------------
        ...
        for ((node_idx = 0; node_idx < TOTAL_NODES; node_idx++)); do
            ...
            for ((local_dp = 0; local_dp < DP_SIZE_LOCAL; local_dp++)); do
                ...
                launch_on_node "${node}" "${local_ip}" "${is_headless}" "0" "${dp_rank}" "${DP_SIZE_LOCAL}" "${local_ip}" "1" "${port}"
            done
        done
    fi
}
