...
# 2. 分布式并行配置 (核心调整)
# ------------------------------------------------------------------------------
# Kimi-K2 采用 MoE 架构，参数量巨大（总计约 600B+），在 128 NPU 集群上，
# 我们采用张量并行(TP)结合流水线并行(PP)以及专家并行(EP)的组合。
# - TP (Tensor Parallel): 推荐节点内 TP=8，以利用高速的节点内互联 (HCCL)
# - PP (Pipeline Parallel): 推荐 PP=16 (16个节点间流水线)，61层 / 16 ≈ 3-4层/阶段
...
EXPERT_PARALLEL_SIZE="${EXPERT_PARALLEL_SIZE:-128}" # 默认让EP=TP*PP或让框架自动切分专家

# ------------------------------------------------------------------------------
# 3. 内存与量化配置
# ------------------------------------------------------------------------------
DTYPE="${DTYPE:-bfloat16}"
QUANTIZATION="${QUANTIZATION:-fp8}" # 模型 config.json 已经指定了 quant_method: fp8
...
ENFORCE_EAGER="${ENFORCE_EAGER:-0}" # 如果支持 CUDA Graph/编译图，则设为0，否则设为1
